...
export interface RegexCheatSheetProps {
  isExpanded: boolean;
  onToggle: () => void;
  /** Optional examples to show (for context-specific examples) */
  examples?: ExampleItem[];
  /** Whether to show replacement column in examples */
...
const defaultExamples: ExampleItem[] = [
  ...
  {
    title: "Match file prefix",
    find: "^IMG_",
    example: "IMG_001.jpg → matches 'IMG_'",
  },
  {
    title: "Match extension",
    find: "\\.bak$",
    example: "data.bak → matches '.bak'",
  },
];
...
const RegexCheatSheet: Component<RegexCheatSheetProps> = (props) => {
  ...
  const examples = () => props.examples ?? defaultExamples;
  const tipText = () =>
    props.tipText ??
    "Use capture groups (parentheses) to match parts of the pattern";
      ...
      <Show when={props.isExpanded}>
        <div class="border-t border-base-300 p-4 space-y-4">
              ...
              <For each={showAllPatterns() ? allPatterns : quickPatterns}>
                {(item) => (
                  <div class="flex items-center gap-2 text-xs p-1.5 rounded bg-base-200 hover:bg-base-300 transition-colors">
                    ...
                    <span class="text-base-content/80 truncate text-xs">
                      {item.description}
                    </span>
                  </div>
                )}
              ...
              <For each={examples()}>
                {(ex) => (
                  <div class="p-2 bg-base-200 rounded text-xs">
                    <div class="font-medium text-base-content/90 mb-1">
                      {ex.title}
                    </div>
                    <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs">
                      <span>
                        <span class="text-base-content/60">Pattern: </span>
                        <code class="font-mono text-accent">{ex.find}</code>
                      </span>
                      <Show when={props.showReplacement && ex.replace !== undefined}>
                        <span>
                          <span class="text-base-content/60">Replace: </span>
                          <code class="font-mono text-secondary">
                            {ex.replace || "(empty)"}
                          </code>
                        </span>
                      </Show>
          ...
          <div class="text-xs text-base-content/60 border-t border-base-300 pt-3">
            <span class="font-medium">Tip:</span> {tipText()}
          </div>
        </div>
      </Show>
    </div>
